...
class ImageFile_DB(__File):
    ...
    def validate_file_format(cls, value: FileFormat) -> FileFormat:
        ...
        if value not in FileFormat.get_image_formats():
            raise ValueError("Invalid file format for ImageFile: %s. Must be one of %s." % (value, FileFormat.get_image_formats()))
        return value
...
class TextFile_DB(__File):
    ...
    def validate_file_format(cls, value: FileFormat) -> FileFormat:
        ...
        if value not in FileFormat.get_text_formats():
            raise ValueError("Invalid file format for TextFile: %s. Must be one of %s." % (value, FileFormat.get_text_formats()))
        return value
...
class AudioFile_DB(__File):
    ...
    def validate_file_format(cls, value: FileFormat) -> FileFormat:
        ...
        if value not in FileFormat.get_audio_formats():
            raise ValueError("Invalid file format for AudioFile: %s. Must be one of %s." % (value, FileFormat.get_audio_formats()))
        return value
...
class UploadedFileResponse(BaseModel):
    ...
    size_in_bytes: Optional[int] = Field(default=0, description="The size of the uploaded file in bytes.")
