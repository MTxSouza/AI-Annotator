...
def test_create_audio_file_record(
    client: TestClient,
    audio_project_payload: dict,
    list_audio_file_payload: list[tuple[str, tuple[str, bytes, str]]],
    ...
    project_response = client.post(url="/projects/", json=audio_project_payload)
    ...
    # Create file record.
    file_response = client.post(url=f"/projects/{project_id}/files/", files=list_audio_file_payload)
    ...
    assert len(file_response_json["data"]) == len(list_audio_file_payload)

    file_id_list = []
    for i, file_data in enumerate(iterable=file_response_json["data"]):
        assert file_data["status"] == "Created"
        assert file_data["message"] == f"Audio file '{list_audio_file_payload[i][1][0]}' uploaded successfully."
    ...
    project = project_response.json()
    assert project["details"]["number_of_files"] == len(list_audio_file_payload)
...
def test_create_duplicate_audio_file_record(
    ...
    duplicate_file_response = client.post(url=f"/projects/{project_id}/files/", files=list_audio_file_payload)
    ...
    assert len(duplicate_file_response_json["data"]) == len(list_audio_file_payload)
    for i, file_data in enumerate(iterable=duplicate_file_response_json["data"]):
        assert file_data["status"] == "Skipped"
        assert file_data["message"] == f"File '{list_audio_file_payload[i][1][0]}' already exists."
